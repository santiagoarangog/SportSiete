...
//      el_div.innerHTML="Disponible $ " + apostadoMaximo;
            el_div.innerHTML = "Disponible $ " + infoUsuario.Usuario["disponible"];
        }
    }
    if ((el_div = el_id("SaldoDisponible")) != null) {
    ...
    if ((el_div = el_id("DetalleItemsApostados")) != null) {
        ...
        tmp_txt += "Paga : " + (Math.floor(totalPagado * 100) / 100);
...
function functionRecharge() {
    console.log("Llego aquí");
    let el_div = el_id("AreaTrabajo");
    el_div.innerHTML = `Ingresa a nuestro sitio web especializado para hacer tus tramites de recargas
                        <br><br>Por favor ingrese al siguiente link para confirmar la transaccion<br><br><br>
                        <a class='button-recharge' href='https://www.sportsiete.com/recharge.html'>Sitio Web Recargas Online <strong>SportSiete.com</strong></a>`;
}
...
function f_js_ListarEncuentros() {
    ...
    switch (v_js_deporte) {
        case 1 :
            ...
            break;
        default :
            nombreDeporte = "Deporte : " + v_js_deporte;
            break;
    }
...
function f_js_dibujarApostado() {
    ...
    if (!v_cwf_id) {
        TableInfoPerfil.innerHTML =
            ...
            + "<tr>"
            + "<td style='cursor:pointer;' onClick='f_js_Autenticar(document.frm_global.SSUsuario.value,document.frm_global.SSClave.value)' title='Ingresar a la plataforma'>Entrar</td>"
    ...
    TableInfoPerfil.innerHTML = "<tr>"
        ...
        + "Apuesta : <input type=text name=InputMontoApostado onKeyUp='apostadoMonto=parseFloat(this.value);f_js_dibujarAreaApuesta()' style='width:3.5em;'>"
        + "</div>"
        + "<div id=ZonaTextoMontoPagado style='display:none;font-size:0.8em;'>"
        + "A Pagar : <div id=TextoMontoPagado style='display:inline;'></div>"
        + "</div>"
        + "</td>"
        + "</tr>"
        + "<tr>"
        + "<td align=center id='ZonaBotonesApostado'>"
        + "<input type=button value='Guardar' onClick='f_js_ApuestaGuardar()'>"
        + "</td>"
        + "</tr>";
    f_js_dibujarAreaApuesta();
}
...
function f_js_DibujarEncuentroDetalle() {
    ...
    var txtEncuentros = "<div class='AreaPaginaCompleta BackgroundBotonAmarilloDegrade' style='letter-spacing:1em;font-style:normal;font-weight:700;vertical-align:middle;'>"
        ...
        + "Liga : " + UltimoListadoEncuentros[n].liga
...
//		console.log("Encuentro no seleccionado " + EncuentroSeleccionado);
    ...
    txtEncuentros += ""
        ...
        */
        + "<div class='AreaPaginaCompleta BackgroundBarraGris' style='text-align:right;height:auto;font-style:normal;font-weight:700;vertical-align:middle;background-color:rgba(188,198,195,0.7);'>"
        + "<input type=button value='Regresar' onClick='f_js_CambiarPaginaActual(\"Encuentros\")' style='border-radius:14px;padding:7px 15px 7px 15px;outline:none;'>"
        + "</div>"
    ;
    divTrabajo.innerHTML = txtEncuentros;
}
...
function f_js_RegistroCrear() {
    if (document.frm_global.Login.value.trim() == "") {
        alert("Debe ingresar una Nombre de usuario");
        return;
    }
    ...
    if (document.frm_global.EMail.value.trim() != document.frm_global.EMail.value.trim()) {
        alert("Los dos correos electrónicos deben ser iguales");
        return;
    }
...
function f_js_PagoIniciar_Respuesta() {
    ...
    el_div.innerHTML = `Transaccion en proceso...
                        <br><br>Por favor ingrese al siguiente link para confirmar la transaccion<br><br><br>
                        <a class='button-recharge' href='https://www.sportsiete.com/recharge.html'>Sitio Web Recargas Online <strong>SportSiete.com</strong></a>`;
//     var json_resp = f_js_procesar_respuesta_generica(cliente_http, true);
//     if (json_resp) {
...
//             el_div.innerHTML = "Transaccion en proceso...<br><br>Por favor ingrese al siguiente link para confirmar la transaccion<br><br><br>"
//                 + "<a class='button-recharge' href='https://www.sportsiete.com/recharge.html'>Sitio Web Recargas Online <strong>SportSiete.com</strong></a>";
//             return;
//         } else {
//             console.log(json_resp);
...
function f_js_DibujarRecarga() {
    ...
    */
    ...
    txtEncuentros += "<div class='AreaPaginaCompleta BackgroundBarraGris' style='text-align:right;height:auto;font-style:normal;font-weight:700;vertical-align:middle;background-color:rgba(188,198,195,0.7);'>"
        //	+				"<input type=button value='Cancelar' onClick='f_js_CambiarPaginaActual(\"Encuentros\")' style='border-radius:14px;padding:7px 15px 7px 15px;outline:none;'>"
        //	+				"<input type=button value='Crear Cuenta' onClick='f_js_CambiarPaginaActual(\"RegistroGrabar\")' style='border-radius:14px;padding:7px 15px 7px 15px;outline:none;'>"
        + "<input type=button value='Carga Temporal' onClick='f_js_GrabarRecarga(5000)' style='border-radius:14px;padding:7px 15px 7px 15px;outline:none;'>"
        + "</div>"
    ;
    divTrabajo.innerHTML = txtEncuentros;
    paypal.Buttons(
...
//			console.log("deporte : futbol");
            ...
            return pRes;
        default:
            console.log("deporte desconocido : " + pDepo);
...
function f_js_ApuestaGuardar() {
    if (apostadoMaximo < apostadoMonto) {
        alert("Saldo Insuficiente, debe recargar");
